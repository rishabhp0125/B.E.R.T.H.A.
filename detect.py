...
Models such as YOLOv5x improve performance, however, may not be compatible with microprocessors such
as the Raspberry Pi 4B used in our project.
...
For slower hardware, we reccomend modifying parameters such as iou-thres and max_det
...
#playing audio file through Bluetooth on the Raspberry Pi
def speak(text):
    tts = gTTS(text)
    tts.save("temp.mp3")
...
def run(
    ...
    show_video = True  # Variable to toggle video feed
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    label = names[c]  # Only the name of the detected object
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        c = int(cls)  # integer class
                        display_label = None if hide_labels else (names[c] if hide_conf else f"{names[c]} {conf:.2f}")
                        annotator.box_label(xyxy, display_label, color=colors(c, True))
                    if save_crop:
                        save_one_box(xyxy, imc, file=save_dir / "crops" / names[c] / f"{p.stem}.jpg", BGR=True)

                    # Text-to-Speech
                    speak(f"There is a {label} nearby.")

            # Stream results
            im0 = annotator.result()
            if view_img and show_video:
                ...
                # Toggle video feed on keypress 'v'
...
#Parameters used to configure the YOLOv5 model
def parse_opt():
